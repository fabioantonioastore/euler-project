...
def get_maximum_path_sum(position = (1, 0), total=75):
    if position[0] == triangule_len:
        ...
        yield from get_maximum_path_sum((position[0] + 1, position[1]), total + triangule[position[0]][position[1]])
        yield from get_maximum_path_sum((position[0] + 1, position[1] + 1), total + triangule[position[0]][position[1] + 1])
